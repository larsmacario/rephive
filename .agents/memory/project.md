...
- **iOS-App**: Capacitor 8 (WKWebView) — dieselbe Vite/React-App aus `dist/`; Xcode-Projekt unter `ios/`; nach Web-Changes: `npm run build && npx cap sync ios`. `ios/App/App/Info.plist`: Foto-Privacy-Strings für Body Tracker; kein `NSAllowsArbitraryLoads` (Standard-ATS für Supabase, rephive.app, YouTube).
...
- `src/components/PlanDayAccordion.tsx` — aufklappbare Plan-Tage mit `WorkoutExercisePreview` (Accordion-Kontext)
...
- Trainingspläne: `plans` + `plan_days` (Workout oder Ruhetag pro Tag); ein aktiver Plan steuert Home-Screen sequenziell. Bearbeiten/Detail: horizontale Folien statt vertikaler Akkordeon-Liste; Detail-Folie 0 = Übersicht.
...
- KI-Trainingspläne: Wizard (`AITrainingPlanWizard.tsx`, 13 Steps inkl. Muskel-Priorität + **KI-Einwilligung** vor Generierung) → `generate-training-plan` (JWT); IAP/StoreKit für v1.1 geplant. **Consent:** `preferences.aiConsent` (Anthropic, versioniert) — Wizard + Einstellungen (Widerruf/Re-Consent); Defense-in-Depth: Client-Gate (`db.ts`) + Server-Gate (`requireAiConsent` in Edge Function). Anamnesis: `musclePriorities` (1–5). Übungsanzahl via `exerciseCountBounds`; Cardio-Warm-up zählt nicht mit. Katalog-Sync im Hintergrund (`EdgeRuntime.waitUntil`). Client-Timeout 150 s. Deploy: `./scripts/deploy-training-plan.sh` (Supabase CLI).
