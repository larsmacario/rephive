...
- `src/lib/planBlocks.ts` — 4 Bausteine (warmup/skill/strength/metcon): Labels, Guide-Hints, Gruppierung, Session-Filter, `disabledBlocks`
...
- Trainingspläne: `plans` + `plan_days` mit **`enabled_blocks`** (aktive Bausteine) + Übungen in `plan_day_exercises` inkl. **`block_type`** + optionalem `catalog_exercise_id`. Keine Workout-Vorlagen mehr.
- Sessions: aggregierte Stats in `sessions` + Satz-Snapshot in `session_exercises` (inkl. **`block_type`**); optional `plan_day_id`; **`skipped_blocks`** = Bausteine nur für diese Session übersprungen (Plan bleibt unverändert).
...
- KI-Trainingspläne: Wizard → `generate-training-plan` liefert pro Tag **`blocks[]`** (warmup/skill/strength/metcon) + `enabledBlocks`; Prompt: 4-Bausteine.md + Individualisierung (Altersbänder). Client mappt in `db.ts` (`blockType`, `enabledBlocks`). Übungsanzahl-Cap gilt **strength-Block** (`exerciseCountBounds` + Alters-Modifier). Deploy: `./scripts/deploy-training-plan.sh`.
